...
export default function DashboardPage() {
  ...
  const handleSaveCompany = async () => {
    ...
    try {
      if (showCustomTypeInput && customTypeLabel.trim()) {
         ...
         if (typeError && typeError.code !== '23505') throw new Error(`Type Creation Error: ${typeError.message}`);
  ...
  const overviewPayroll = isAdmin ? salaryPayments
  ...
  const selectedCompanyEmployeeIds = selectedCompany ? employees.filter(e => 
    e.company_id === selectedCompany.id || 
    (e.company_roles && e.company_roles.some((r: any) => r.company_id === selectedCompany.id))
  ).map(e => e.id) : [];
  ...
  const getActivityFeed = () => {
    ...
    const isCompanyMatch = (itemCompanyId: any) => {
      if (isAdmin) {
        if (activeWorkspace) return itemCompanyId?.toString() === activeWorkspace?.toString();
        return activityFilterCompanyId === 'all' || itemCompanyId?.toString() === activityFilterCompanyId;
      }
      return itemCompanyId?.toString() === activeCompanyId?.toString();
    };

    projects.forEach(p => {
      if (!isAdmin && !(p.assignee_ids || []).includes(employeeId)) return;
      if (isAdmin && !isCompanyMatch(p.company_id)) return;
      const compName = companies.find(c => c.id === p.company_id)?.name || 'Subsidiary';
      feed.push({ id: `proj_${p.id}`, type: 'Project', title: isAdmin ? `Project Initiated: ${p.name}` : `Assigned to Project: ${p.name}`, desc: p.description || 'A new workflow was created.', date: p.created_at || todayStr, companyName: compName, icon: Briefcase, color: 'text-blue-500 bg-blue-50 border-blue-200' });
    });

    salaryPayments.forEach(p => {
      if (!isAdmin && p.employee_id !== employeeId) return;
      const compId = p.company_id || employees.find(e => e.id === p.employee_id)?.company_id;
      if (isAdmin && !isCompanyMatch(compId)) return;
      const emp = employees.find(e => e.id === p.employee_id);
      const compName = companies.find(c => c.id === compId)?.name || 'Subsidiary';
      feed.push({ id: `pay_${p.id}`, type: 'Payment', title: `Payout Disbursed: ₹${parseFloat(p.amount || 0).toLocaleString()}`, desc: isAdmin ? `Transferred to ${emp?.name || 'Employee'} (${p.payment_type || 'Standard'})` : `Your ${p.payment_type || 'payout'} payment was processed.`, date: p.created_at || p.payment_date || todayStr, companyName: compName, icon: Wallet, color: 'text-emerald-500 bg-emerald-50 border-emerald-200' });
    });

    announcements.forEach(a => {
      if (!isAdmin && a.company_id && a.company_id?.toString() !== activeCompanyId?.toString()) return;
      if (isAdmin && !isCompanyMatch(a.company_id)) return;
        ...
        {visibleAnnouncements.length > 0 && (
          <div className="bg-blue-50 border border-blue-100 rounded-2xl sm:rounded-3xl p-4 sm:p-5 relative overflow-hidden shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              <Bell className="h-3.5 w-3.5 sm:h-4 w-4 text-blue-600"/>
              ...
              <div className="p-4 sm:p-6 flex-1 overflow-y-auto flex flex-col max-sm:[&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select value={announcementForm.company_id} onChange={e => setAnnouncementForm({...announcementForm, company_id: e.target.value})} className="w-full h-10 sm:h-11 rounded-xl border border-slate-200 bg-white px-3 sm:px-4 text-[12px] sm:text-[13px] font-medium outline-none focus:border-blue-500 shadow-sm">
                        {isAdmin && <option value="all">General (All Companies)</option>}
                        {(isAdmin ? overviewCompanies : [currentCompany]).filter(Boolean).map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                    </div>
                    <div>
                ...
                <div className="mt-auto">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {announcements.filter(a => isAdmin || a.company_id?.toString() === activeCompanyId?.toString()).length === 0 && <p className="text-[11px] sm:text-sm text-slate-400 italic">No active announcements.</p>}
                    {announcements.filter(a => isAdmin || a.company_id?.toString() === activeCompanyId?.toString()).map(ann => (
                ...
                {companyModalMode === 'view' && selectedCompany ? (
                  <div className="flex-1 flex flex-col space-y-5 sm:space-y-6 pb-2 sm:pb-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                      ...
                      <div className="col-span-2 sm:col-span-1 bg-slate-50 border border-slate-100 rounded-xl sm:rounded-xl p-3 sm:p-4">
                         <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Payroll</p>
                         <p className="text-lg sm:text-2xl font-bold text-slate-800 tracking-tight">
                           ₹{selectedCompanyPayroll.toLocaleString()}
                         </p>
                      </div>
                    </div>
